...
{
    auto NewBuff = static_cast<PWCH>(ExAllocatePoolWithTag(GetPoolType(USBDK_NON_PAGED_POOL),
                                                           NewLenBytes,
                                                           'SUHR'));
    if (NewBuff == nullptr)
    {
...
NTSTATUS CString::Create(PCUNICODE_STRING String)
{
    ...
    m_String.Buffer = static_cast<PWCH>(ExAllocatePoolWithTag(GetPoolType(USBDK_NON_PAGED_POOL),
...
PVOID DuplicateStaticBuffer(const void *Buffer, SIZE_T Length, POOL_TYPE PoolType)
{
    ...
    auto Duplicate = ExAllocatePoolWithTag(GetPoolType(PoolType), Length, 'BDHR');
